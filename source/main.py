...
# -------------------- НАСТРОЙКИ --------------------
# Вставь свой токен ниже вместо текста внутри кавычек
GITHUB_TOKEN = "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" 
REPO_NAME = "KiryaScript/white-lists"

if GITHUB_TOKEN and "ВСТАВЬ" not in GITHUB_TOKEN:
    g = Github(auth=Auth.Token(GITHUB_TOKEN))
else:
    # Если токен не вставлен, скрипт попытается работать без него (но пушить не сможет)
    print("⚠️ Токен не обнаружен или не заменен! Вставь токен в переменную GITHUB_TOKEN.")
    g = Github()
